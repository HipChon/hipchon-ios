...
class HomeViewModel {
    ...
    let selectedHashtag = PublishRelay<Hashtag>()
    let selectedBanner = PublishRelay<Banner>()
    let bannerCurrentIdx = BehaviorRelay<Int>(value: 1)

    init() {
        hashtags = Driver.just(Hashtag.model)
        ...
        let bannerData = BehaviorSubject<[Banner]>(value: [])
