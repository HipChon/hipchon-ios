...
class MemoViewModel {
    ...
    init(_ place: BehaviorSubject<PlaceModel>) {
        ...
        let memo = BehaviorSubject<MemoModel>(value: MemoModel())
        ...
        inputContent
            ...
            .withLatestFrom(memo) { MemoModel(content: $0, color: $1.color ?? "") }
            .bind(to: memo)
            .disposed(by: bag)
        ...
        changedColor
            .distinctUntilChanged()
            .withLatestFrom(memo) { MemoModel(content: $1.content ?? "나만의 메모를 남겨볼까요", color: $0.memoColorString()) }
            .bind(to: memo)
            .disposed(by: bag)
        ...
        completeButtonTapped
            ...
            .map { ($0.0, MemoModel(content: $0.1, color: $0.2.memoColorString())) }
